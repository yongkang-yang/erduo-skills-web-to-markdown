...
- **原文至上**：翻译必须基于原文逐段逐句进行，严禁总结、概括、压缩或改写原文内容。不得省略句子或用自己的话重述。段落可以根据标点与分段优化规则重新拆分（允许比原文多分段），但不得合并原文的多个段落为一段。原文的所有标题必须保留，不得省略或合并。如果通过 URL 抓取的内容与原文有出入（被摘要化、截断或改写），必须想办法获取完整原文后再翻译
